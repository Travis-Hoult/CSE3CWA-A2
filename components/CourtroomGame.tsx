...
export default function CourtroomGame() {
  ...
  function triggerVerdict(category?: Category | string | null, verdict?: string | null) {
    if (navigatingRef.current) return;
    navigatingRef.current = true;
    clearAllTimers();

    try {
      sessionStorage.setItem("verdict", JSON.stringify({
  ...
  useEffect(() => {
    ...
    return () => { if (tickerRef.current) { window.clearInterval(tickerRef.current); tickerRef.current = null; } };
  }, [gameStarted]);
  ...
  useEffect(() => {
    ...
    return () => { if (messageIntervalRef.current) { window.clearInterval(messageIntervalRef.current); messageIntervalRef.current = null; } };
  ...
  function onFix() { if (top?.id && top.id === penaltyTaskId) resetPenalty(); popTop(); }
  ...
  return (
    ...
    >
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div
          aria-live="polite"
          style={{ ...headerBadge, minWidth: 140, textAlign: "center", fontSize: 30, fontVariantNumeric: "tabular-nums" }}
        >
          ⏱ {mmss}
        </div>
      </header>
      ...
      {!gameStarted && (
        <section
          ...
          style={{
            ...
            background: "rgba(255,255,255,0.65)", zIndex: 15,
          }}
        >
          <div
            style={{
              background: "rgba(255,255,255,0.97)",
              border: "1px solid #ccc", borderRadius: 12, padding: 24, width: 360, textAlign: "center",
              boxShadow: "0 12px 24px rgba(0,0,0,0.15)",
            }}
          >
            <h2 style={{ marginTop: 0 }}>Ready to start?</h2>
            <p style={{ margin: "8px 0 16px" }}>Press start to begin the 40:00 timer and receive messages.</p>
            <button
              onClick={startGame}
              style={{ fontSize: 18, padding: "10px 16px", borderRadius: 8, border: "2px solid #111", background: "#111", color: "#fff", cursor: "pointer" }}
            >
              Start
            </button>
          </div>
      ...
      {gameStarted && top && (
        <div
          role="dialog" aria-modal="true" aria-label="Incoming message"
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.2)", display: "grid", placeItems: "center", padding: 16, zIndex: 20 }}
        >
          <div
            style={{
              background: "#fff", border: "1px solid #ccc", borderRadius: 12, padding: 20,
              width: "min(92vw, 560px)", boxShadow: "0 12px 24px rgba(0,0,0,0.15)", textAlign: "center",
            }}
          >
            <div style={{ fontWeight: 900, letterSpacing: 1, color: top.critical ? "#b00000" : "#333", marginBottom: 8 }}>
            ...
            {top.critical ? (
              <>
                {!criticalReviewOpen ? (
                  <button
                    ...
                    style={{
                      width: "100%", maxWidth: 280, margin: "0 auto", display: "inline-block",
                      padding: "10px 14px", borderRadius: 8,
                      border: "2px solid #b00000", background: "#ffebeb", color: "#b00000", fontWeight: 800, cursor: "pointer",
                    }}
                  ...
                  <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                    <button onClick={onFix}           style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #111", background: "#fff" }}>Fix</button>
                    <button onClick={onIgnoreCritical} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #aaa", background: "#fff" }}>Ignore</button>
                  </div>
                )}
              </>
            ) : (
              <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                <button onClick={onFix}              style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #111", background: "#fff" }}>Fix</button>
                <button onClick={onSnooze}           style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #777", background: "#f5f5f5" }}>Snooze</button>
                <button onClick={onIgnoreNonCritical}style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #aaa", background: "#fff" }}>Ignore</button>
              </div>
            )}
          </div>
        </div>
...
<pre>${JSON.stringify(summary, null, 2)}</pre>
          ...
          style={{ fontSize: 16, padding: "10px 14px", borderRadius: 8, border: "2px solid #111", background: "#fff", cursor: "pointer" }}
        >
          Save Output
        </button>
      </div>
